...
including BRD Phase-2 macro-factors.
"""
...
def build_route(src, dst, stops_val):
    iv = [c for c in CITIES if c!=src and c!=dst]
    if stops_val=="non-stop": return f"{src} - {dst}"
    if stops_val=="1 stop":   return f"{src} - {np.random.choice(iv)} - {dst}"
    v1,v2 = np.random.choice(iv,2,replace=False)
    return f"{src} - {v1} - {v2} - {dst}"


def generate(n_rows: int = 100_000, seed: int = 42) -> pd.DataFrame:
    """Generate n_rows synthetic flight records. Returns 25-column DataFrame."""
    np.random.seed(seed)
    N = n_rows
    ...
    stops_a  = np.random.choice(STOPS,   N, p=STOPS_W)
    ...
    routes = [build_route(s,d,st) for s,d,st in zip(sources,dests,stops_a)]
    ...
    restr  = np.array([1 if (s,d) in RESTRICTED or (d,s) in RESTRICTED else 0 for s,d in zip(sources,dests)])
    ...
    lhours = np.where(stops_a=="non-stop",0,np.where(stops_a=="1 stop",np.random.randint(1,8,N),np.random.randint(4,14,N)))
    ...
    p *= np.where(stops_a=="non-stop",1.08,np.where(stops_a=="1 stop",0.95,0.88))
    ...
    return pd.DataFrame({
        ...
        "Journey_Month":jmonth,"Journey_Day":jday,"Total_Stops":stops_a,"Class":classes,
